/*
  # Fix regex parsing in find_claim_photo

  The claim resolver must strip filename extensions before deriving the
  16-digit claim code from `storage_path` / `storage.objects.name`.
  This migration aligns regex escaping with Postgres (single backslash).
*/
